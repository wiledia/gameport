          ...
          @if(Auth::user()->id == $offer->user_id)
            ...
            <div class="flex-center">
                ...
                @if(config('settings.paypal'))
                    {{-- PayPal button --}}
                    <a href="{{url('offer/'.$offer->id.'/pay')}}" class="btn btn-lg btn-success m-r-5" id="pay-now-button"><i class="fa fa-paypal"></i> <span class="hidden-xs-down"> PayPal</span></a>
                @endif
                @if(config('settings.stripe'))
                    {{-- Stripe button --}}
                    <a href="javascript:void(0)" class="btn btn-lg btn-success" id="stripeCheckout"><i class="fa fa-cc-stripe"></i> <span class="hidden-xs-down"> Stripe</span></a>
                @endif
                </div>
            </div>
          @else
...
document.getElementById('stripeCheckout').addEventListener('click', function(e) {
  ...
  handler.open({
    ...
    opened: function() {
      $('#stripeCheckout').html('<i class="fa fa-cc-stripe"></i> Stripe');
