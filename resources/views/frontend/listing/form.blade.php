      ...
      <div class="flex-center">
        ...
        <div>
          <span class="selected-game-title">
            <strong>{{$game->name}}</strong><span class="release-year m-l-5">{{$game->release_date->format('Y')}}</span>
          </span>
...
@if(config('settings.payment'))
  ...
  var payment_fees = function(price) {
    var new_price = price;
    var fees = (new_price * {{ config('settings.variable_fee')/100 }}) @if(config('settings.fixed_fee')) + {{ config('settings.fixed_fee') }} @endif;
