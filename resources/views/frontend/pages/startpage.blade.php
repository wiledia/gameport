...
<div class="m-b-20" style="font-size: 24px; color: #fff; font-weight: 700; "><i class="fa fa-tags" aria-hidden="true"></i> {{ trans('listings.general.newest_listings') }}</div>
...
{{-- End modal for filter options --}}
  ...
  <div class="row">
    ...
    {{-- Start filtered listings --}}
    @foreach($listings_filtered as $listing)
    {{-- START GAME --}}
    <div class="col-xs-6 col-sm-4 col-md-3 col-lg-3 col-xl-2 m-b-20">

      {{-- Start Game Cover --}}
      <div class="card game-cover-wrapper hvr-grow-shadow filtered">
        {{-- Pacman Loader for background image - show only when cover exists --}}
        @if($listing->game->image_cover)
        <div class="loader pacman-loader cover-loader"></div>
        {{-- Show game name, when no cover exist --}}
    ...
    {{-- End GAME --}}
    @endforeach
    {{-- End filtered listings --}}

    {{-- Show more link on bottom --}}
    @if(count($listings) + count($listings_filtered) == 24)
