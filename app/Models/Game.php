...
class Game extends Model
{
    ...
    */
    ...
    {
        return $this->hasMany('App\Models\Listing')->orderBy('price')->where('status', null)->whereHas('user', function ($query) {
    ...
    public function listingsCount()
    {
        return $this->hasOne('App\Models\Listing')
            ...
            ->groupBy('game_id')->where('status', null)->whereHas('user', function ($query) {
                $query->where('status', 1);
            })->orWhere('status', 0)->whereHas('user', function ($query) {
                $query->where('status', 1);
            });
    ...
    public function cheapestListing()
    {
        return $this->hasOne('App\Models\Listing')
            ...
            ->groupBy('game_id')->where('status', null)->where('sell', 1)->whereHas('user', function ($query) {
                $query->where('status', 1);
            })->orWhere('status', 0)->where('sell', 1)->whereHas('user', function ($query) {
                $query->where('status', 1);
            });
    }
    ...
    public function averagePrice()
    {
        return $this->hasOne('App\Models\Listing')
            ...
            ->groupBy('game_id')->where('status', '>', '0')->where('sell', 1)->whereHas('user', function ($query) {
                $query->where('status', 1);
            });
    }
